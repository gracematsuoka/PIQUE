...
export const fetchBoardPosts = async (boardId, cursor) => {
    const token = await auth.currentUser.getIdToken();
...
export const removePost = async (boardId, postId) => {
    const token = await auth.currentUser.getIdToken();
    await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/boardposts/${postId}/remove-post/${boardId}`, {
        method: 'DELETE',
        headers: {
            Authorization: `Bearer ${token}`
        }
...
export const addPost = async (boardId, postId) => {
    const token = await auth.currentUser.getIdToken();
    await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/boardposts/${postId}/add-post/${boardId}`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${token}`
        }
